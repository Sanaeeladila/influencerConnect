...
<body>
    ...
    <h1>Voici la liste de tous les partenariats</h1>
        <?php
            ...
            $req = "SELECT * FROM partenariat WHERE marque = (SELECT fname FROM users WHERE unique_id = {$getid})";
            $getfname = mysqli_query($conn, $req);
            if(mysqli_num_rows($getfname) > 0){
                while($drow = mysqli_fetch_assoc($getfname)){
                    $sql = "SELECT * FROM users WHERE fname = '{$drow['influenceur']}'";
                    $getInf = mysqli_query($conn, $sql);
                    if(mysqli_num_rows($getInf) > 0){
                        $row = mysqli_fetch_assoc($getInf);
                        ?>
                        ...
                        <div class="row">
                            ...
                            <p><span><?= $row['fname'] . " " . $row['lname'] ; ?></span></p>
                        ...
                        /* echo "L'influenceur n'est pas trouvé!"; */
